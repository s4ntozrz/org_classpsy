...
tabBtns.forEach(btn => {
    btn.addEventListener('click', () => {
        tabBtns.forEach(b => b.className = 'tab-btn bg-white/5 border border-white/10 px-4 py-1.5 rounded-full text-xs font-bold text-zinc-400 hover:text-white transition flex items-center gap-1');
...
// ESTADOS DE EDIÇÃO (Controlam qual ID está sendo editado)
let editIds = { aluno: null, aviso: null, evento: null, material: null, horario: null, contato: null };

// Função Auxiliar para Modo Edição
...
async function carregarAlunos() {
    ...
    try {
        ...
        list.forEach(item => {
            ...
            card.innerHTML = `
                <div>
                    <h4 class="font-bold text-white text-sm">${item.nome || "Sem Nome"}</h4>
                    <p class="text-[11px] text-zinc-400">Matrícula: <span class="text-[#D4FF00] font-semibold">${item.matricula}</span></p>
                </div>
                <div class="flex gap-1">
                    <button onclick="window.editarAluno('${item.id}', '${item.nome}', '${item.matricula}')" class="w-8 h-8 bg-blue-500/10 text-blue-400 rounded-full flex items-center justify-center hover:bg-blue-500/30 transition shrink-0"><i class="ph-bold ph-pencil-simple text-sm"></i></button>
...
window.editarAluno = (id, nome, matricula) => {
    editIds.aluno = id;
    document.getElementById('nome-aluno').value = nome;
    document.getElementById('matricula-aluno').value = matricula;
    toggleEditMode('form-aluno', 'btn-submit-aluno', 'btn-cancel-aluno', true);
}
...
document.getElementById('form-aluno').addEventListener('submit', async (e) => {
    e.preventDefault();
    const nome = document.getElementById('nome-aluno').value; const matricula = document.getElementById('matricula-aluno').value;
    try {
...
async function carregarAvisos() {
    ...
    try {
        ...
        if (snap.empty) return lista.innerHTML = '<p class="text-zinc-500 text-xs text-center mt-2">Nenhum aviso.</p>';
        lista.innerHTML = '';
        let list = []; snap.forEach(d => list.push({ id: d.id, ...d.data() }));
        ...
        list.forEach(item => {
            ...
            card.innerHTML = `
                <div class="flex-1 pr-2">
                    <h4 class="font-bold text-white text-sm truncate">${item.titulo}</h4>
                    <p class="text-[11px] text-zinc-400">Tipo: <span class="uppercase">${item.urgencia}</span></p>
                </div>
                <div class="flex gap-1">
                    <button onclick="window.editarAviso('${item.id}', '${item.titulo.replace(/'/g, "\\'")}', '${item.texto.replace(/'/g, "\\'").replace(/\n/g, "\\n")}', '${item.urgencia}')" class="w-8 h-8 bg-blue-500/10 text-blue-400 rounded-full flex items-center justify-center hover:bg-blue-500/30 transition shrink-0"><i class="ph-bold ph-pencil-simple text-sm"></i></button>
...
window.editarAviso = (id, titulo, texto, urgencia) => {
    editIds.aviso = id;
    document.getElementById('titulo-aviso').value = titulo;
    document.getElementById('texto-aviso').value = texto;
    document.getElementById('urgencia-aviso').value = urgencia;
    toggleEditMode('form-aviso', 'btn-submit-aviso', 'btn-cancel-aviso', true);
}
...
document.getElementById('form-aviso').addEventListener('submit', async (e) => {
    e.preventDefault();
    const titulo = document.getElementById('titulo-aviso').value; const texto = document.getElementById('texto-aviso').value; const urgencia = document.getElementById('urgencia-aviso').value;
    try {
...
async function carregarEventos() {
    ...
    try {
        ...
        if (snap.empty) return lista.innerHTML = '<p class="text-zinc-500 text-xs text-center mt-2">Nenhum evento.</p>';
        lista.innerHTML = '';
        let list = []; snap.forEach(d => list.push({ id: d.id, ...d.data() }));
        ...
        list.forEach(item => {
            ...
            card.innerHTML = `
                <div class="flex-1 pr-2">
                    <h4 class="font-bold text-white text-sm truncate">${item.titulo}</h4>
                    <p class="text-[11px] text-zinc-400">${item.data} - <span class="uppercase">${item.tipo}</span></p>
                </div>
                <div class="flex gap-1">
                    <button onclick="window.editarEvento('${item.id}', '${item.titulo.replace(/'/g, "\\'")}', '${item.data}', '${item.tipo}')" class="w-8 h-8 bg-blue-500/10 text-blue-400 rounded-full flex items-center justify-center hover:bg-blue-500/30 transition shrink-0"><i class="ph-bold ph-pencil-simple text-sm"></i></button>
...
window.editarEvento = (id, titulo, data, tipo) => {
    editIds.evento = id;
    document.getElementById('titulo-evento').value = titulo; document.getElementById('data-evento').value = data; document.getElementById('tipo-evento').value = tipo;
    toggleEditMode('form-evento', 'btn-submit-evento', 'btn-cancel-evento', true);
}
...
if(formEvento) {
    formEvento.addEventListener('submit', async (e) => {
        e.preventDefault();
        const titulo = document.getElementById('titulo-evento').value; const data = document.getElementById('data-evento').value; const tipo = document.getElementById('tipo-evento').value;
        try {
...
document.getElementById('btn-importar-csv').addEventListener('click', () => {
    ...
    if (!fileInput.files.length) return alert("Selecione um CSV");
...
async function carregarMateriais() {
    ...
    try {
        ...
        if (snap.empty) return lista.innerHTML = '<p class="text-zinc-500 text-xs text-center mt-2">Nenhum material.</p>';
        lista.innerHTML = '';
        let list = []; snap.forEach(d => list.push({ id: d.id, ...d.data() }));
        ...
        list.forEach(item => {
            ...
            card.innerHTML = `
                <div class="flex-1 pr-2">
                    <h4 class="font-bold text-white text-sm truncate">${item.titulo}</h4>
                    <p class="text-[11px] text-zinc-400">Disciplina: <span class="text-indigo-400">${item.materia}</span></p>
                </div>
                <div class="flex gap-1">
                    <button onclick="window.editarMaterial('${item.id}', '${item.materia.replace(/'/g, "\\'")}', '${item.titulo.replace(/'/g, "\\'")}', '${item.link}')" class="w-8 h-8 bg-blue-500/10 text-blue-400 rounded-full flex items-center justify-center hover:bg-blue-500/30 transition shrink-0"><i class="ph-bold ph-pencil-simple text-sm"></i></button>
...
window.editarMaterial = (id, materia, titulo, link) => {
    editIds.material = id;
    document.getElementById('materia-material').value = materia; document.getElementById('titulo-material').value = titulo; document.getElementById('link-material').value = link;
    toggleEditMode('form-material', 'btn-submit-material', 'btn-cancel-material', true);
}
...
if(formMaterial) {
    formMaterial.addEventListener('submit', async (e) => {
        e.preventDefault();
        const materia = document.getElementById('materia-material').value; const titulo = document.getElementById('titulo-material').value; const link = document.getElementById('link-material').value;
        try {
...
async function carregarHorarios() {
    ...
    try {
        ...
        if (snap.empty) return lista.innerHTML = '<p class="text-zinc-500 text-xs text-center mt-2">Nenhuma aula na grade.</p>';
        lista.innerHTML = '';
        let list = []; snap.forEach(d => list.push({ id: d.id, ...d.data() }));
        ...
        list.forEach(aula => {
            ...
            card.innerHTML = `
                ...
                <div class="flex gap-1">
                    <button onclick="window.editarHorario('${aula.id}', '${aula.dia}', '${aula.materia}', '${aula.hora}', '${aula.prof}')" class="w-8 h-8 bg-blue-500/10 text-blue-400 rounded-full flex items-center justify-center hover:bg-blue-500/30 transition shrink-0"><i class="ph-bold ph-pencil-simple text-sm"></i></button>
...
window.editarHorario = (id, dia, materia, hora, prof) => {
    editIds.horario = id;
    document.getElementById('dia-horario').value = dia; document.getElementById('materia-horario').value = materia; document.getElementById('hora-horario').value = hora; document.getElementById('prof-horario').value = prof;
    toggleEditMode('form-horario', 'btn-submit-horario', 'btn-cancel-horario', true);
}
...
if(formHorario) {
    formHorario.addEventListener('submit', async (e) => {
        e.preventDefault();
        const dia = document.getElementById('dia-horario').value; const materia = document.getElementById('materia-horario').value; const hora = document.getElementById('hora-horario').value; const prof = document.getElementById('prof-horario').value;
        try {
...
async function carregarContatos() {
    ...
    try {
        ...
        if (snap.empty) return lista.innerHTML = '<p class="text-zinc-500 text-xs text-center mt-2">Nenhum contato salvo.</p>';
        lista.innerHTML = '';
        let list = []; snap.forEach(d => list.push({ id: d.id, ...d.data() }));
        list.sort((a, b) => a.nome.localeCompare(b.nome));
        list.forEach(contato => {
            ...
            card.innerHTML = `
                ...
                <div class="flex gap-1">
                    <button onclick="window.editarContato('${contato.id}', '${contato.nome}', '${contato.cargo}', '${contato.numero}')" class="w-8 h-8 bg-blue-500/10 text-blue-400 rounded-full flex items-center justify-center hover:bg-blue-500/30 transition shrink-0"><i class="ph-bold ph-pencil-simple text-sm"></i></button>
...
window.editarContato = (id, nome, cargo, numero) => {
    editIds.contato = id;
    document.getElementById('nome-contato').value = nome; document.getElementById('cargo-contato').value = cargo; document.getElementById('numero-contato').value = numero;
    toggleEditMode('form-contato', 'btn-submit-contato', 'btn-cancel-contato', true);
}
...
if(formContato) {
    formContato.addEventListener('submit', async (e) => {
        e.preventDefault();
        const nome = document.getElementById('nome-contato').value; const cargo = document.getElementById('cargo-contato').value; const numero = document.getElementById('numero-contato').value; 
        try {
